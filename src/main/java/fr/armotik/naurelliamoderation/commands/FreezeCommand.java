...
public class FreezeCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender commandSender, Command command, String s, String[] strings) {
        ...
        if (target.hasPermission("naurellia.staff.helper")) {
            commandSender.sendMessage(Louise.getName() + "§cYou can't freeze a staff member");
            return false;
        }

        if (!target.isOnline()) {
            commandSender.sendMessage(Louise.getName() + "§cThis player is not online");
            return false;
        }

        EventManager.getFrozen().put(target.getUniqueId(), true);

        commandSender.sendMessage(Louise.getName() + "§aYou have frozen §e" + target.getName());
